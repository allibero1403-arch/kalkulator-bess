...
st.markdown("### 1. Nakłady inwestycyjne (CAPEX)")
c1, c2 = st.columns(2)
with c1:
...
st.markdown("### 3. Koszty operacyjne (OPEX)")
o1, o2 = st.columns(2)
with o1:
...
# --- BLOK 4: PRZYCHODY ---
st.markdown("### 4. Strumienie przychodów")
r1, r2 = st.columns(2)
with r1:
...
# --- LOGIKA FINANSOWA ---
kwota_kredytu = float(capex_pln * (lewar / 100))
equity = float(capex_pln - kwota_kredytu)
oprocentowanie = float((wibor + marza) / 100)
ebitda = float(suma_p - (10 * opex_mw))

# Rata kredytu (10 lat)
n_lat = 10
if oprocentowanie > 0:
    rata = float(kwota_kredytu * (oprocentowanie / (1 - (1 + oprocentowanie)**-n_lat)))
else:
    rata = float(kwota_kredytu / n_lat)

dscr = float(ebitda / rata) if rata > 0 else 0.0
...
st.markdown("---")
st.markdown("### 5. Wyniki i rentowność projektu")
m1, m2, m3 = st.columns(3)
m1.metric("EBITDA", f"{ebitda:,.0f} PLN")
m2.metric("Wskaźnik DSCR", f"{dscr:.2f}")
m3.metric("IRR (15 lat)", f"{irr*100:.2f}%" if irr is not None else "N/A")
